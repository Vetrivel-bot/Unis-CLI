...
export const registerMessageEvents = (socket, ctx) => {
  ...
  // Make sure you pass { database, localPrivateKeyB64 } when you call registerMessageEvents.
  const database = ctx?.database ?? null;
  const localPrivateKeyB64 = ctx?.localPrivateKeyB64 ?? ctx?.privateKey ?? null;
  ...
  socket.on('chat_message', async message => {
    ...
    if (database) {
      try {
        // pass localPrivateKeyB64 so saveIncomingMessage can attempt decrypt before saving plaintext
        await saveIncomingMessage(database, message, { localPrivateKeyB64 });
